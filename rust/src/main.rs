...
use utils::camera_utils::{handle_camera_motion, CameraState, handle_motion};
use utils::key_events::{KeyState, handle_key_events, process_key_events};
...
fn main() {
    ...
    let mut sphereMesh = mesh::Mesh::new(&display, "../models/sphere_ico_low.obj", true, true, false);
    let mut cubeContainerMesh = mesh::Mesh::new(&display, "../models/cube.obj", true, true, true);
    let mut sphereContainerMesh = mesh::Mesh::new(&display, "../models/sphere.obj", true, true, true);
    ...
    sim.create_sphere(Vector3::new(0.0, 0.0, 0.0), 0.15, Vector3::new(0.0, 0.0, 0.0), Vector3::new(0.0, 0.0, 0.0), false);
    sim.create_sphere(Vector3::new(4.0, 0.0, 0.0), 0.15, Vector3::new(0.0, 0.0, 0.0), Vector3::new(0.0, 0.0, 0.0), false);
    ...
    let _ = event_loop.run(move |event, window_target| {
        ...
        if elapsed >= Duration::from_secs_f64(1.0 / TARGET_FPS as f64) {
            unsafe {
                if let Some(last_frame_time) = LAST_FRAME_TIME {
                    ...
                    if duration >= Duration::from_secs(1) {
                        ...
                        window.set_title(&format!("FPS: {:.2}, NUM_PARTICLES: {}", FPS, sim.spheres.len()));
                    }
                } else {
                    LAST_FRAME_TIME = Some(now);
                }
            ...
            let container_trait_vec: Vec<Arc<RwLock<dyn ContainerTrait>>> = sim.sphere_containers
            .iter()
        ...
        match event {
            glium::winit::event::Event::WindowEvent { event, .. } => {
                handle_camera_motion(&event, &camera, &mut last_cursor_pos, &right_mouse_button_pressed, &camera_state);
